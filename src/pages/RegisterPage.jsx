...
const RegisterPage = () => {
  ...
  // Debug logging for email confirmation state
  console.log('RegisterPage: Render - emailConfirmationSent:', emailConfirmationSent, 'confirmationEmail:', confirmationEmail);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result && result.needsEmailConfirmation) {
        console.log('RegisterPage: Email confirmation required for:', result.email);
        console.log('RegisterPage: Setting emailConfirmationSent to true');
        
        setEmailConfirmationSent(true);
        setConfirmationEmail(result.email || registrationData.email);
        setRetryCount(0); // Reset retry count on success
        
        // Add a small delay to ensure state is set
        setTimeout(() => {
          console.log('RegisterPage: State after setting confirmation:', {
            emailConfirmationSent: true,
            confirmationEmail: result.email || registrationData.email
          });
        }, 100);
        
      } else if (result && result.user) {
        // Direct registration success (shouldn't happen with email confirmation enabled)
        console.log('RegisterPage: Direct registration success, navigating to dashboard');
        navigate('/dashboard');
      } else {
  ...
  // Show email confirmation screen if confirmation was sent
  if (emailConfirmationSent) {
    console.log('RegisterPage: Rendering email confirmation screen for:', confirmationEmail);
    return (
